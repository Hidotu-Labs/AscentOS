...
# kernel/linux-subset.txt into kernel/linux/ (gitignored).  It also writes
...
SUBSET_FILE="$ROOT/kernel/linux-subset.txt"
FILES_FILE="$ROOT/kernel/linux-files.txt"
...
# ── Generate Makefile.files from kernel/linux-files.txt ──────────────────────
mapfile -t srcfiles < <(grep -v '^[[:space:]]*\(#\|$\)' "$FILES_FILE")
[ "${#srcfiles[@]}" -gt 0 ] || die "kernel/linux-files.txt lists no source files"
...
done
{
    ...
    echo "# Configure the file list in kernel/linux-files.txt."
    printf 'linux-obj-y := \\\n'
    last=$((${#srcfiles[@]} - 1))
