...
static uint64_t sys_ioctl(uint64_t fd, uint64_t request, uint64_t arg,
    ...
    if (t) {
        klog_puts("[IOCTL] tid="); klog_uint64(t->tid);
        klog_puts(" fd=");         klog_uint64(fd);
        klog_puts(" request=0x");  klog_hex32(request);
        klog_puts(" arg=0x");      klog_uint64(arg);
        klog_puts("\n");
    }

    klog_puts("[SYSCALL] sys_ioctl ENTER fd="); klog_uint64(fd);
    klog_puts(" request=0x"); klog_hex32((uint32_t)request);
    klog_puts(" arg=0x");     klog_hex64(arg);
    klog_puts("\n");
    ...
    if (fd < MAX_FDS && t->fds[fd]) {
        ...
        klog_puts("[SYSCALL] ioctl: node name="); klog_puts(node->name);
        klog_puts(" has_ioctl="); klog_uint64(node->ioctl ? 1 : 0);
        klog_puts("\n");
        ...
        if (node->ioctl) {
            if (request & 0xC0000000) {
                ...
                if (sz > 0 && !vmm_is_user_addr_range_valid(arg, sz)) {
                    klog_puts("[SYSCALL] ioctl: invalid arg pointer for encoded request\n");
                    return (uint64_t)-14;
                }
            }
    ...
    case VT_ACTIVATE:
    case VT_WAITACTIVE:
    case VT_DISALLOCATE: ret = 0; break;
    case KDSETMODE:
    case KDSKBMODE:      ret = 0; break;
    case KDGETMODE: {
        int *mode = (int *)arg;
        if (!mode) { ret = (uint64_t)-14; break; }
        *mode = KD_TEXT; ret = 0; break;
    }
    case KDGKBMODE: {
        int *mode = (int *)arg;
        if (!mode) { ret = (uint64_t)-14; break; }
    ...
    case 0x5451: ret = 0; break; // KDSIGACCEPT
    case TIOCGETD: {
        int *ldisc = (int *)arg;
        if (!ldisc) { ret = (uint64_t)-14; break; }
        *ldisc = 0; ret = 0; break;
    }
    default:
