...
void drm_run_phase3_test(void) {
    ...
    ver.name = name; ver.name_len = 32;
    ver.date = date; ver.date_len = 32;
    ver.desc = desc; ver.desc_len = 64;
    ...
    list_for_each_entry(mobj, &global_drm_dev.kms_objects, list) {
        kms_count++;
    }
    ...
    cd.width = 128; cd.height = 128; cd.bpp = 32;
    if (node->ioctl(node, DRM_IOCTL_MODE_CREATE_DUMB, (uint64_t)&cd) == 0) {
        ...
        if (node->ioctl(node, DRM_IOCTL_MODE_MAP_DUMB, (uint64_t)&md) == 0) {
            ...
            if (node->mmap) {
                uint64_t addr = node->mmap(node, 0, cd.size, 3 /* PROT_READ|PROT_WRITE */, 0, md.offset);
                if (addr != (uint64_t)-1) {
