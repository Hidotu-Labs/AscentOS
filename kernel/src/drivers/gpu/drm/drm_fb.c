...
static int drm_fb_validate(struct drm_device *dev,
    ...
    for (int p = 0; p < info->num_planes; p++) {
        ...
        if (!drm_gem_find_by_handle(dev, cmd->handles[p])) {
 ...
 */
struct drm_framebuffer *drm_framebuffer_create_full(struct drm_device *dev,
                                                    struct drm_mode_fb_cmd2 *cmd) {
    ...
    if (drm_fb_validate(dev, cmd, info) != 0)
        return NULL;
    ...
    fb->bpp   = info->cpp[0] * 8;
    fb->pitch = cmd->pitches[0];

    for (int p = 0; p < info->num_planes; p++) {
        fb->gem_obj[p] = drm_gem_find_by_handle(dev, cmd->handles[p]);
    ...
    return (struct drm_framebuffer *)fb;
}
 ...
 */
int drm_ioctl_addfb2(struct drm_device *dev, uint64_t arg) {
