...
static const struct drm_property_def drm_prop_catalogue[] = {
    ...
    { DRM_PROP_ID_MODE_ID,      DRM_PROP_FLAG_ATOMIC | DRM_PROP_TYPE_BLOB,"MODE_ID",     0, UINT32_MAX },
...
#define PROP_CATALOGUE_SIZE (sizeof(drm_prop_catalogue) / sizeof(drm_prop_catalogue[0]))
...
struct drm_prop_blob *drm_blob_create(struct drm_device *dev,
                                      const void *data, uint32_t length) {
    struct drm_prop_blob *blob = kmalloc(sizeof(struct drm_prop_blob));
    if (!blob) return NULL;

    blob->data = kmalloc(length);
    if (!blob->data) { kfree(blob); return NULL; }
    ...
    case DRM_MODE_OBJECT_PLANE: {
        ...
        case DRM_PROP_ID_FB_ID: {
            ...
            list_for_each_entry(cobj, &dev->kms_objects, list) {
                if (cobj->type != DRM_MODE_OBJECT_CRTC) continue;
        ...
        case DRM_PROP_ID_SRC_X: case DRM_PROP_ID_SRC_Y:
        case DRM_PROP_ID_SRC_W: case DRM_PROP_ID_SRC_H:
        case DRM_PROP_ID_CRTC_X: case DRM_PROP_ID_CRTC_Y:
        case DRM_PROP_ID_CRTC_W: case DRM_PROP_ID_CRTC_H:
...
int drm_ioctl_atomic(struct vfs_node *node, struct drm_file *file,
    ...
    for (uint32_t i = 0; i < req->count_objs; i++) {
        ...
        struct drm_mode_object *iter;
        list_for_each_entry(iter, &dev->kms_objects, list) {
            if (iter->id == obj_id) { mobj = iter; break; }
        }
        if (!mobj) {
            spinlock_release(&dev->lock);
