#include "font.h"

// 8x16 bitmap font for printable ASCII (32-126).
// Each glyph is 16 bytes; each byte is one row, MSB = leftmost pixel.
static const uint8_t font_data[][FONT_HEIGHT] = {
    // 32: ' ' (space)
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    // 33: '!'
    {0x00,0x00,0x18,0x3C,0x3C,0x3C,0x18,0x18,0x18,0x00,0x18,0x18,0x00,0x00,0x00,0x00},
    // 34: '"'
    {0x00,0x66,0x66,0x66,0x24,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    // 35: '#'
    {0x00,0x00,0x00,0x6C,0x6C,0xFE,0x6C,0x6C,0xFE,0x6C,0x6C,0x00,0x00,0x00,0x00,0x00},
    // 36: '$'
    {0x00,0x10,0x7C,0xD6,0xD0,0x70,0x1C,0x16,0xD6,0x7C,0x10,0x00,0x00,0x00,0x00,0x00},
    // 37: '%'
    {0x00,0x00,0x00,0x00,0xC2,0xC6,0x0C,0x18,0x30,0x66,0xC6,0x00,0x00,0x00,0x00,0x00},
    // 38: '&'
    {0x00,0x00,0x38,0x6C,0x6C,0x38,0x76,0xDC,0xCC,0xCC,0x76,0x00,0x00,0x00,0x00,0x00},
    // 39: '\''
    {0x00,0x30,0x30,0x30,0x60,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    // 40: '('
    {0x00,0x00,0x0C,0x18,0x30,0x30,0x30,0x30,0x30,0x18,0x0C,0x00,0x00,0x00,0x00,0x00},
    // 41: ')'
    {0x00,0x00,0x30,0x18,0x0C,0x0C,0x0C,0x0C,0x0C,0x18,0x30,0x00,0x00,0x00,0x00,0x00},
    // 42: '*'
    {0x00,0x00,0x00,0x00,0x66,0x3C,0xFF,0x3C,0x66,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    // 43: '+'
    {0x00,0x00,0x00,0x00,0x18,0x18,0x7E,0x18,0x18,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    // 44: ','
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x18,0x18,0x30,0x00,0x00,0x00,0x00},
    // 45: '-'
    {0x00,0x00,0x00,0x00,0x00,0x00,0xFE,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    // 46: '.'
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x18,0x18,0x00,0x00,0x00,0x00,0x00},
    // 47: '/'
    {0x00,0x00,0x02,0x06,0x0C,0x18,0x30,0x60,0xC0,0x80,0x00,0x00,0x00,0x00,0x00,0x00},
    // 48: '0'
    {0x00,0x00,0x7C,0xC6,0xCE,0xDE,0xF6,0xE6,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 49: '1'
    {0x00,0x00,0x18,0x38,0x78,0x18,0x18,0x18,0x18,0x18,0x7E,0x00,0x00,0x00,0x00,0x00},
    // 50: '2'
    {0x00,0x00,0x7C,0xC6,0x06,0x0C,0x18,0x30,0x60,0xC6,0xFE,0x00,0x00,0x00,0x00,0x00},
    // 51: '3'
    {0x00,0x00,0x7C,0xC6,0x06,0x06,0x3C,0x06,0x06,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 52: '4'
    {0x00,0x00,0x0C,0x1C,0x3C,0x6C,0xCC,0xFE,0x0C,0x0C,0x1E,0x00,0x00,0x00,0x00,0x00},
    // 53: '5'
    {0x00,0x00,0xFE,0xC0,0xC0,0xFC,0x06,0x06,0x06,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 54: '6'
    {0x00,0x00,0x38,0x60,0xC0,0xC0,0xFC,0xC6,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 55: '7'
    {0x00,0x00,0xFE,0xC6,0x06,0x0C,0x18,0x30,0x30,0x30,0x30,0x00,0x00,0x00,0x00,0x00},
    // 56: '8'
    {0x00,0x00,0x7C,0xC6,0xC6,0xC6,0x7C,0xC6,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 57: '9'
    {0x00,0x00,0x7C,0xC6,0xC6,0xC6,0x7E,0x06,0x06,0x0C,0x78,0x00,0x00,0x00,0x00,0x00},
    // 58: ':'
    {0x00,0x00,0x00,0x00,0x18,0x18,0x00,0x00,0x18,0x18,0x00,0x00,0x00,0x00,0x00,0x00},
    // 59: ';'
    {0x00,0x00,0x00,0x00,0x18,0x18,0x00,0x00,0x18,0x18,0x30,0x00,0x00,0x00,0x00,0x00},
    // 60: '<'
    {0x00,0x00,0x06,0x0C,0x18,0x30,0x60,0x30,0x18,0x0C,0x06,0x00,0x00,0x00,0x00,0x00},
    // 61: '='
    {0x00,0x00,0x00,0x00,0x00,0x7E,0x00,0x00,0x7E,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    // 62: '>'
    {0x00,0x00,0x60,0x30,0x18,0x0C,0x06,0x0C,0x18,0x30,0x60,0x00,0x00,0x00,0x00,0x00},
    // 63: '?'
    {0x00,0x00,0x7C,0xC6,0xC6,0x0C,0x18,0x18,0x00,0x18,0x18,0x00,0x00,0x00,0x00,0x00},
    // 64: '@'
    {0x00,0x00,0x7C,0xC6,0xC6,0xDE,0xDE,0xDE,0xDC,0xC0,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 65: 'A'
    {0x00,0x00,0x10,0x38,0x6C,0xC6,0xC6,0xFE,0xC6,0xC6,0xC6,0x00,0x00,0x00,0x00,0x00},
    // 66: 'B'
    {0x00,0x00,0xFC,0x66,0x66,0x66,0x7C,0x66,0x66,0x66,0xFC,0x00,0x00,0x00,0x00,0x00},
    // 67: 'C'
    {0x00,0x00,0x3C,0x66,0xC2,0xC0,0xC0,0xC0,0xC2,0x66,0x3C,0x00,0x00,0x00,0x00,0x00},
    // 68: 'D'
    {0x00,0x00,0xF8,0x6C,0x66,0x66,0x66,0x66,0x66,0x6C,0xF8,0x00,0x00,0x00,0x00,0x00},
    // 69: 'E'
    {0x00,0x00,0xFE,0x66,0x62,0x68,0x78,0x68,0x62,0x66,0xFE,0x00,0x00,0x00,0x00,0x00},
    // 70: 'F'
    {0x00,0x00,0xFE,0x66,0x62,0x68,0x78,0x68,0x60,0x60,0xF0,0x00,0x00,0x00,0x00,0x00},
    // 71: 'G'
    {0x00,0x00,0x3C,0x66,0xC2,0xC0,0xC0,0xDE,0xC6,0x66,0x3A,0x00,0x00,0x00,0x00,0x00},
    // 72: 'H'
    {0x00,0x00,0xC6,0xC6,0xC6,0xC6,0xFE,0xC6,0xC6,0xC6,0xC6,0x00,0x00,0x00,0x00,0x00},
    // 73: 'I'
    {0x00,0x00,0x3C,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x3C,0x00,0x00,0x00,0x00,0x00},
    // 74: 'J'
    {0x00,0x00,0x1E,0x0C,0x0C,0x0C,0x0C,0x0C,0xCC,0xCC,0x78,0x00,0x00,0x00,0x00,0x00},
    // 75: 'K'
    {0x00,0x00,0xE6,0x66,0x6C,0x6C,0x78,0x6C,0x6C,0x66,0xE6,0x00,0x00,0x00,0x00,0x00},
    // 76: 'L'
    {0x00,0x00,0xF0,0x60,0x60,0x60,0x60,0x60,0x62,0x66,0xFE,0x00,0x00,0x00,0x00,0x00},
    // 77: 'M'
    {0x00,0x00,0xC6,0xEE,0xFE,0xFE,0xD6,0xC6,0xC6,0xC6,0xC6,0x00,0x00,0x00,0x00,0x00},
    // 78: 'N'
    {0x00,0x00,0xC6,0xE6,0xF6,0xFE,0xDE,0xCE,0xC6,0xC6,0xC6,0x00,0x00,0x00,0x00,0x00},
    // 79: 'O'
    {0x00,0x00,0x7C,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 80: 'P'
    {0x00,0x00,0xFC,0x66,0x66,0x66,0x7C,0x60,0x60,0x60,0xF0,0x00,0x00,0x00,0x00,0x00},
    // 81: 'Q'
    {0x00,0x00,0x7C,0xC6,0xC6,0xC6,0xC6,0xD6,0xDE,0x7C,0x0C,0x0E,0x00,0x00,0x00,0x00},
    // 82: 'R'
    {0x00,0x00,0xFC,0x66,0x66,0x66,0x7C,0x6C,0x66,0x66,0xE6,0x00,0x00,0x00,0x00,0x00},
    // 83: 'S'
    {0x00,0x00,0x7C,0xC6,0xC6,0x60,0x38,0x0C,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 84: 'T'
    {0x00,0x00,0x7E,0x7E,0x5A,0x18,0x18,0x18,0x18,0x18,0x3C,0x00,0x00,0x00,0x00,0x00},
    // 85: 'U'
    {0x00,0x00,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 86: 'V'
    {0x00,0x00,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0x6C,0x38,0x10,0x00,0x00,0x00,0x00,0x00},
    // 87: 'W'
    {0x00,0x00,0xC6,0xC6,0xC6,0xC6,0xD6,0xD6,0xFE,0x6C,0x6C,0x00,0x00,0x00,0x00,0x00},
    // 88: 'X'
    {0x00,0x00,0xC6,0xC6,0x6C,0x38,0x38,0x38,0x6C,0xC6,0xC6,0x00,0x00,0x00,0x00,0x00},
    // 89: 'Y'
    {0x00,0x00,0x66,0x66,0x66,0x66,0x3C,0x18,0x18,0x18,0x3C,0x00,0x00,0x00,0x00,0x00},
    // 90: 'Z'
    {0x00,0x00,0xFE,0xC6,0x86,0x0C,0x18,0x30,0x62,0xC6,0xFE,0x00,0x00,0x00,0x00,0x00},
    // 91: '['
    {0x00,0x00,0x3C,0x30,0x30,0x30,0x30,0x30,0x30,0x30,0x3C,0x00,0x00,0x00,0x00,0x00},
    // 92: '\\'
    {0x00,0x00,0x80,0xC0,0x60,0x30,0x18,0x0C,0x06,0x02,0x00,0x00,0x00,0x00,0x00,0x00},
    // 93: ']'
    {0x00,0x00,0x3C,0x0C,0x0C,0x0C,0x0C,0x0C,0x0C,0x0C,0x3C,0x00,0x00,0x00,0x00,0x00},
    // 94: '^'
    {0x10,0x38,0x6C,0xC6,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    // 95: '_'
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0x00,0x00,0x00,0x00},
    // 96: '`'
    {0x30,0x30,0x18,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    // 97: 'a'
    {0x00,0x00,0x00,0x00,0x00,0x78,0x0C,0x7C,0xCC,0xCC,0x76,0x00,0x00,0x00,0x00,0x00},
    // 98: 'b'
    {0x00,0x00,0xE0,0x60,0x60,0x78,0x6C,0x66,0x66,0x66,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 99: 'c'
    {0x00,0x00,0x00,0x00,0x00,0x7C,0xC6,0xC0,0xC0,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 100: 'd'
    {0x00,0x00,0x1C,0x0C,0x0C,0x3C,0x6C,0xCC,0xCC,0xCC,0x76,0x00,0x00,0x00,0x00,0x00},
    // 101: 'e'
    {0x00,0x00,0x00,0x00,0x00,0x7C,0xC6,0xFE,0xC0,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 102: 'f'
    {0x00,0x00,0x1C,0x36,0x32,0x30,0x7C,0x30,0x30,0x30,0x78,0x00,0x00,0x00,0x00,0x00},
    // 103: 'g'
    {0x00,0x00,0x00,0x00,0x00,0x76,0xCC,0xCC,0xCC,0x7C,0x0C,0xCC,0x78,0x00,0x00,0x00},
    // 104: 'h'
    {0x00,0x00,0xE0,0x60,0x60,0x6C,0x76,0x66,0x66,0x66,0xE6,0x00,0x00,0x00,0x00,0x00},
    // 105: 'i'
    {0x00,0x00,0x18,0x18,0x00,0x38,0x18,0x18,0x18,0x18,0x3C,0x00,0x00,0x00,0x00,0x00},
    // 106: 'j'
    {0x00,0x00,0x06,0x06,0x00,0x0E,0x06,0x06,0x06,0x06,0x66,0x66,0x3C,0x00,0x00,0x00},
    // 107: 'k'
    {0x00,0x00,0xE0,0x60,0x60,0x66,0x6C,0x78,0x6C,0x66,0xE6,0x00,0x00,0x00,0x00,0x00},
    // 108: 'l'
    {0x00,0x00,0x38,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x3C,0x00,0x00,0x00,0x00,0x00},
    // 109: 'm'
    {0x00,0x00,0x00,0x00,0x00,0xEC,0xFE,0xD6,0xD6,0xD6,0xD6,0x00,0x00,0x00,0x00,0x00},
    // 110: 'n'
    {0x00,0x00,0x00,0x00,0x00,0xDC,0x66,0x66,0x66,0x66,0x66,0x00,0x00,0x00,0x00,0x00},
    // 111: 'o'
    {0x00,0x00,0x00,0x00,0x00,0x7C,0xC6,0xC6,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 112: 'p'
    {0x00,0x00,0x00,0x00,0x00,0xDC,0x66,0x66,0x66,0x7C,0x60,0x60,0xF0,0x00,0x00,0x00},
    // 113: 'q'
    {0x00,0x00,0x00,0x00,0x00,0x76,0xCC,0xCC,0xCC,0x7C,0x0C,0x0C,0x1E,0x00,0x00,0x00},
    // 114: 'r'
    {0x00,0x00,0x00,0x00,0x00,0xDC,0x76,0x66,0x60,0x60,0xF0,0x00,0x00,0x00,0x00,0x00},
    // 115: 's'
    {0x00,0x00,0x00,0x00,0x00,0x7C,0xC6,0x70,0x1C,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00},
    // 116: 't'
    {0x00,0x00,0x10,0x30,0x30,0xFC,0x30,0x30,0x30,0x36,0x1C,0x00,0x00,0x00,0x00,0x00},
    // 117: 'u'
    {0x00,0x00,0x00,0x00,0x00,0xCC,0xCC,0xCC,0xCC,0xCC,0x76,0x00,0x00,0x00,0x00,0x00},
    // 118: 'v'
    {0x00,0x00,0x00,0x00,0x00,0xC6,0xC6,0xC6,0x6C,0x38,0x10,0x00,0x00,0x00,0x00,0x00},
    // 119: 'w'
    {0x00,0x00,0x00,0x00,0x00,0xC6,0xC6,0xD6,0xD6,0xFE,0x6C,0x00,0x00,0x00,0x00,0x00},
    // 120: 'x'
    {0x00,0x00,0x00,0x00,0x00,0xC6,0x6C,0x38,0x38,0x6C,0xC6,0x00,0x00,0x00,0x00,0x00},
    // 121: 'y'
    {0x00,0x00,0x00,0x00,0x00,0xC6,0xC6,0xC6,0xC6,0x7E,0x06,0x0C,0xF8,0x00,0x00,0x00},
    // 122: 'z'
    {0x00,0x00,0x00,0x00,0x00,0xFE,0xCC,0x18,0x30,0x66,0xFE,0x00,0x00,0x00,0x00,0x00},
    // 123: '{'
    {0x00,0x00,0x0E,0x18,0x18,0x18,0x70,0x18,0x18,0x18,0x0E,0x00,0x00,0x00,0x00,0x00},
    // 124: '|'
    {0x00,0x00,0x18,0x18,0x18,0x18,0x00,0x18,0x18,0x18,0x18,0x00,0x00,0x00,0x00,0x00},
    // 125: '}'
    {0x00,0x00,0x70,0x18,0x18,0x18,0x0E,0x18,0x18,0x18,0x70,0x00,0x00,0x00,0x00,0x00},
    // 126: '~'
    {0x00,0x76,0xDC,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
};

#define FONT_GLYPH_COUNT (sizeof(font_data) / sizeof(font_data[0]))

static const uint8_t fallback_glyph[FONT_HEIGHT] = {
    0xFF,0x81,0x81,0x81,0x81,0x81,0x81,0x81,0x81,0x81,0x81,0x81,0xFF,0x00,0x00,0x00
};

// Unicode Block Elements (U+2580 – U+2595)
static const uint8_t block_elements[][FONT_HEIGHT] = {
    // U+2580 ▀ UPPER HALF BLOCK
    {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    // U+2581 ▁ LOWER ONE EIGHTH BLOCK
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF},
    // U+2582 ▂ LOWER ONE QUARTER BLOCK
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF},
    // U+2583 ▃ LOWER THREE EIGHTHS BLOCK
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF},
    // U+2584 ▄ LOWER HALF BLOCK
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF},
    // U+2585 ▅ LOWER FIVE EIGHTHS BLOCK
    {0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF},
    // U+2586 ▆ LOWER THREE QUARTERS BLOCK
    {0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF},
    // U+2587 ▇ LOWER SEVEN EIGHTHS BLOCK
    {0x00,0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF},
    // U+2588 █ FULL BLOCK
    {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF},
    // U+2589 ▉ LEFT SEVEN EIGHTHS BLOCK
    {0xFE,0xFE,0xFE,0xFE,0xFE,0xFE,0xFE,0xFE,0xFE,0xFE,0xFE,0xFE,0xFE,0xFE,0xFE,0xFE},
    // U+258A ▊ LEFT THREE QUARTERS BLOCK
    {0xFC,0xFC,0xFC,0xFC,0xFC,0xFC,0xFC,0xFC,0xFC,0xFC,0xFC,0xFC,0xFC,0xFC,0xFC,0xFC},
    // U+258B ▋ LEFT FIVE EIGHTHS BLOCK
    {0xF8,0xF8,0xF8,0xF8,0xF8,0xF8,0xF8,0xF8,0xF8,0xF8,0xF8,0xF8,0xF8,0xF8,0xF8,0xF8},
    // U+258C ▌ LEFT HALF BLOCK
    {0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0,0xF0},
    // U+258D ▍ LEFT THREE EIGHTHS BLOCK
    {0xE0,0xE0,0xE0,0xE0,0xE0,0xE0,0xE0,0xE0,0xE0,0xE0,0xE0,0xE0,0xE0,0xE0,0xE0,0xE0},
    // U+258E ▎ LEFT ONE QUARTER BLOCK
    {0xC0,0xC0,0xC0,0xC0,0xC0,0xC0,0xC0,0xC0,0xC0,0xC0,0xC0,0xC0,0xC0,0xC0,0xC0,0xC0},
    // U+258F ▏ LEFT ONE EIGHTH BLOCK
    {0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80},
    // U+2590 ▐ RIGHT HALF BLOCK
    {0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F},
    // U+2591 ░ LIGHT SHADE
    {0x22,0x00,0x88,0x00,0x22,0x00,0x88,0x00,0x22,0x00,0x88,0x00,0x22,0x00,0x88,0x00},
    // U+2592 ▒ MEDIUM SHADE
    {0xAA,0x55,0xAA,0x55,0xAA,0x55,0xAA,0x55,0xAA,0x55,0xAA,0x55,0xAA,0x55,0xAA,0x55},
    // U+2593 ▓ DARK SHADE
    {0xDD,0xFF,0x77,0xFF,0xDD,0xFF,0x77,0xFF,0xDD,0xFF,0x77,0xFF,0xDD,0xFF,0x77,0xFF},
    // U+2594 ▔ UPPER ONE EIGHTH BLOCK
    {0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    // U+2595 ▕ RIGHT ONE EIGHTH BLOCK
    {0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01},
};

// Box Drawing Characters (subset of U+2500–U+257F)
// 8x16 pixel bitmaps.  The "pen" is 2 pixels wide so lines are visible.
static const uint8_t box_2500[FONT_HEIGHT] = // HORIZONTAL
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t box_2501[FONT_HEIGHT] = // ━ HEAVY HORIZONTAL
    {0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t box_2502[FONT_HEIGHT] = // │ VERTICAL
    {0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18};
static const uint8_t box_2503[FONT_HEIGHT] = // ┃ HEAVY VERTICAL
    {0x3C,0x3C,0x3C,0x3C,0x3C,0x3C,0x3C,0x3C,0x3C,0x3C,0x3C,0x3C,0x3C,0x3C,0x3C,0x3C};
static const uint8_t box_250C[FONT_HEIGHT] = // ┌ DOWN AND RIGHT
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x1F,0x1F,0x18,0x18,0x18,0x18,0x18,0x18,0x18};
static const uint8_t box_2510[FONT_HEIGHT] = // ┐ DOWN AND LEFT
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xF8,0xF8,0x18,0x18,0x18,0x18,0x18,0x18,0x18};
static const uint8_t box_2514[FONT_HEIGHT] = // └ UP AND RIGHT
    {0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x1F,0x1F,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t box_2518[FONT_HEIGHT] = // ┘ UP AND LEFT
    {0x18,0x18,0x18,0x18,0x18,0x18,0x18,0xF8,0xF8,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t box_251C[FONT_HEIGHT] = // ├ VERTICAL AND RIGHT
    {0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x1F,0x1F,0x18,0x18,0x18,0x18,0x18,0x18,0x18};
static const uint8_t box_2524[FONT_HEIGHT] = // ┤ VERTICAL AND LEFT
    {0x18,0x18,0x18,0x18,0x18,0x18,0x18,0xF8,0xF8,0x18,0x18,0x18,0x18,0x18,0x18,0x18};
static const uint8_t box_252C[FONT_HEIGHT] = // ┬ HORIZONTAL AND DOWN
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0x18,0x18,0x18,0x18,0x18,0x18,0x18};
static const uint8_t box_2534[FONT_HEIGHT] = // ┴ HORIZONTAL AND UP
    {0x18,0x18,0x18,0x18,0x18,0x18,0x18,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t box_253C[FONT_HEIGHT] = // ┼ CROSS
    {0x18,0x18,0x18,0x18,0x18,0x18,0x18,0xFF,0xFF,0x18,0x18,0x18,0x18,0x18,0x18,0x18};

// Additional border styles used by btop.
static const uint8_t box_254E[FONT_HEIGHT] = // ╎ LIGHT DOUBLE-DASH VERTICAL
    {0x18,0x18,0x18,0x00,0x00,0x18,0x18,0x18,0x00,0x00,0x18,0x18,0x18,0x00,0x00,0x18};
static const uint8_t box_2550[FONT_HEIGHT] = // ═ DOUBLE HORIZONTAL
    {0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0x00,0x00,0xFF,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t box_2551[FONT_HEIGHT] = // ║ DOUBLE VERTICAL
    {0x24,0x24,0x24,0x24,0x24,0x24,0x24,0x24,0x24,0x24,0x24,0x24,0x24,0x24,0x24,0x24};
static const uint8_t box_2554[FONT_HEIGHT] = // ╔
    {0x00,0x00,0x00,0x00,0x00,0x00,0x3F,0x24,0x27,0x20,0x20,0x20,0x20,0x20,0x20,0x20};
static const uint8_t box_2557[FONT_HEIGHT] = // ╗
    {0x00,0x00,0x00,0x00,0x00,0x00,0xFC,0x24,0xE4,0x04,0x04,0x04,0x04,0x04,0x04,0x04};
static const uint8_t box_255A[FONT_HEIGHT] = // ╚
    {0x20,0x20,0x20,0x20,0x20,0x20,0x27,0x24,0x3F,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t box_255D[FONT_HEIGHT] = // ╝
    {0x04,0x04,0x04,0x04,0x04,0x04,0xE4,0x24,0xFC,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t box_2560[FONT_HEIGHT] = // ╠
    {0x20,0x20,0x20,0x20,0x20,0x20,0x27,0x24,0x3F,0x24,0x27,0x20,0x20,0x20,0x20,0x20};
static const uint8_t box_2563[FONT_HEIGHT] = // ╣
    {0x04,0x04,0x04,0x04,0x04,0x04,0xE4,0x24,0xFC,0x24,0xE4,0x04,0x04,0x04,0x04,0x04};
static const uint8_t box_2566[FONT_HEIGHT] = // ╦
    {0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0x24,0xE7,0x24,0x24,0x24,0x24,0x24,0x24,0x24};
static const uint8_t box_2569[FONT_HEIGHT] = // ╩
    {0x24,0x24,0x24,0x24,0x24,0x24,0xE7,0x24,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t box_256C[FONT_HEIGHT] = // ╬
    {0x24,0x24,0x24,0x24,0x24,0x24,0xE7,0x24,0xFF,0x24,0xE7,0x24,0x24,0x24,0x24,0x24};
static const uint8_t box_256D[FONT_HEIGHT] = // ╭
    {0x00,0x00,0x00,0x00,0x00,0x00,0x0C,0x1F,0x1F,0x18,0x18,0x18,0x18,0x18,0x18,0x18};
static const uint8_t box_256E[FONT_HEIGHT] = // ╮
    {0x00,0x00,0x00,0x00,0x00,0x00,0x30,0xF8,0xF8,0x18,0x18,0x18,0x18,0x18,0x18,0x18};
static const uint8_t box_256F[FONT_HEIGHT] = // ╯
    {0x18,0x18,0x18,0x18,0x18,0x18,0x30,0xF8,0xF8,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t box_2570[FONT_HEIGHT] = // ╰
    {0x18,0x18,0x18,0x18,0x18,0x18,0x0C,0x1F,0x1F,0x00,0x00,0x00,0x00,0x00,0x00,0x00};

// Horizontal scan lines (used by VT100 ACS)
static const uint8_t scan_23BA[FONT_HEIGHT] = // ⎺ HORIZONTAL SCAN LINE-1
    {0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t scan_23BB[FONT_HEIGHT] = // ⎻ HORIZONTAL SCAN LINE-3
    {0x00,0x00,0x00,0x00,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t scan_23BC[FONT_HEIGHT] = // ⎼ HORIZONTAL SCAN LINE-7
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0x00,0x00,0x00,0x00};
static const uint8_t scan_23BD[FONT_HEIGHT] = // ⎽ HORIZONTAL SCAN LINE-9
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF};

// Miscellaneous symbols
static const uint8_t glyph_25C6[FONT_HEIGHT] = // ◆ BLACK DIAMOND
    {0x00,0x00,0x00,0x18,0x3C,0x7E,0xFF,0x7E,0x3C,0x18,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t glyph_00B0[FONT_HEIGHT] = // ° DEGREE SIGN
    {0x00,0x1C,0x22,0x22,0x1C,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t glyph_00B1[FONT_HEIGHT] = // ± PLUS-MINUS
    {0x00,0x00,0x18,0x18,0x7E,0x18,0x18,0x00,0x7E,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t glyph_00B7[FONT_HEIGHT] = // · MIDDLE DOT
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x18,0x18,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t glyph_00A3[FONT_HEIGHT] = // £ POUND SIGN
    {0x00,0x1C,0x36,0x30,0x30,0x7C,0x30,0x30,0x30,0x66,0xFC,0x00,0x00,0x00,0x00,0x00};
static const uint8_t glyph_03C0[FONT_HEIGHT] = // π PI
    {0x00,0x00,0x00,0x00,0x00,0x7E,0x24,0x24,0x24,0x24,0x42,0x00,0x00,0x00,0x00,0x00};
static const uint8_t glyph_2260[FONT_HEIGHT] = // ≠ NOT EQUAL TO
    {0x00,0x00,0x04,0x08,0x7E,0x10,0x7E,0x20,0x40,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t glyph_2264[FONT_HEIGHT] = // ≤ LESS THAN OR EQUAL
    {0x00,0x00,0x06,0x18,0x60,0x18,0x06,0x00,0x7E,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
static const uint8_t glyph_2265[FONT_HEIGHT] = // ≥ GREATER THAN OR EQUAL
    {0x00,0x00,0x60,0x18,0x06,0x18,0x60,0x00,0x7E,0x00,0x00,0x00,0x00,0x00,0x00,0x00};

const uint8_t *font_get_glyph(uint32_t codepoint) {
    // ASCII range (32–126)
    if (codepoint >= 32 && codepoint <= 126) {
        return font_data[codepoint - 32];
    }

    // Block Elements (U+2580–U+2595)
    if (codepoint >= 0x2580 && codepoint <= 0x2595) {
        return block_elements[codepoint - 0x2580];
    }

    // Box Drawing (U+2500–U+253C, sparse)
    switch (codepoint) {
    case 0x2500: return box_2500; // ─
    case 0x2501: return box_2501; // ━
    case 0x2502: return box_2502; // │
    case 0x2503: return box_2503; // ┃
    case 0x250C: return box_250C; // ┌
    case 0x2510: return box_2510; // ┐
    case 0x2514: return box_2514; // └
    case 0x2518: return box_2518; // ┘
    case 0x251C: return box_251C; // ├
    case 0x2524: return box_2524; // ┤
    case 0x252C: return box_252C; // ┬
    case 0x2534: return box_2534; // ┴
    case 0x253C: return box_253C; // ┼
    case 0x254E: return box_254E; // ╎
    case 0x2550: return box_2550; // ═
    case 0x2551: return box_2551; // ║
    case 0x2554: return box_2554; // ╔
    case 0x2557: return box_2557; // ╗
    case 0x255A: return box_255A; // ╚
    case 0x255D: return box_255D; // ╝
    case 0x2560: return box_2560; // ╠
    case 0x2563: return box_2563; // ╣
    case 0x2566: return box_2566; // ╦
    case 0x2569: return box_2569; // ╩
    case 0x256C: return box_256C; // ╬
    case 0x256D: return box_256D; // ╭
    case 0x256E: return box_256E; // ╮
    case 0x256F: return box_256F; // ╯
    case 0x2570: return box_2570; // ╰
    // Horizontal scan lines (VT100 ACS)
    case 0x23BA: return scan_23BA; // ⎺
    case 0x23BB: return scan_23BB; // ⎻
    case 0x23BC: return scan_23BC; // ⎼
    case 0x23BD: return scan_23BD; // ⎽
    // Miscellaneous symbols (VT100 ACS)
    case 0x25C6: return glyph_25C6; // ◆
    case 0x00B0: return glyph_00B0; // °
    case 0x00B1: return glyph_00B1; // ±
    case 0x00B7: return glyph_00B7; // ·
    case 0x00A3: return glyph_00A3; // £
    case 0x03C0: return glyph_03C0; // π
    case 0x2260: return glyph_2260; // ≠
    case 0x2264: return glyph_2264; // ≤
    case 0x2265: return glyph_2265; // ≥
    default: break;
    }

    // Unknown codepoint – render a space (invisible) instead of a box.
    // This keeps alignment correct for unsupported Unicode characters.
    if (codepoint > 126) {
        return font_data[0]; // space glyph
    }

    return fallback_glyph;
}
